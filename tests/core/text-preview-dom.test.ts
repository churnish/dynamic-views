...
describe('updateTextPreviewDOM', () => {
  ...
  describe('newline handling', () => {
    it('creates <p> elements when text has paragraph breaks', () => {
      const card = makeEmptyCard();
...
describe('setTextPreviewContent', () => {
  ...
  it('sets plain text and removes has-paragraphs when no newlines', () => {
    const el = makePreviewEl();
  ...
  it('splits double newlines into <p> elements and adds has-paragraphs', () => {
    const el = makePreviewEl();
    setTextPreviewContent(el, 'Para one\n\nPara two\n\nPara three');
    ...
    expect(el.classList.contains('has-paragraphs')).toBe(true);
  });

  it('collapses triple+ newlines into single paragraph break', () => {
    const el = makePreviewEl();
    setTextPreviewContent(el, 'A\n\n\n\nB');
  ...
  it('treats single newline as part of same paragraph', () => {
    const el = makePreviewEl();
    setTextPreviewContent(el, 'Line one\nLine two');
  ...
  it('clears previous paragraphs before setting new ones', () => {
    const el = makePreviewEl();
    setTextPreviewContent(el, 'First\n\nSecond');
    expect(el.children.length).toBe(2);
    setTextPreviewContent(el, 'A\n\nB\n\nC');
    expect(el.children.length).toBe(3);
  ...
  it('removes has-paragraphs when switching from newline to plain text', () => {
    const el = makePreviewEl();
    setTextPreviewContent(el, 'First\n\nSecond');
    expect(el.classList.contains('has-paragraphs')).toBe(true);
    setTextPreviewContent(el, 'plain text');
    expect(el.classList.contains('has-paragraphs')).toBe(false);
    expect(el.textContent).toBe('plain text');
  });
});
