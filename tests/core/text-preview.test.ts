...
import {
  ...
  loadNotePreview,
} from '../../src/core/text-preview';
import { App, TFile } from 'obsidian';

describe('preview', () => {
  describe('sanitizeForTextPreview', () => {
...
More content`;
    ...
    it('should strip bare heading syntax', () => {
      expect(sanitizeForTextPreview('#')).toBe('');
      expect(sanitizeForTextPreview('## ')).toBe('');
      expect(sanitizeForTextPreview('#\nContent here')).toBe('Content here');
    });
...
[X] Bare checked uppercase`;
    ...
    it('should strip checklist items with no trailing text', () => {
      expect(sanitizeForTextPreview('- [ ]')).toBe('');
      expect(sanitizeForTextPreview('- [x]')).toBe('');
      expect(sanitizeForTextPreview('- [-]')).toBe('');
      expect(sanitizeForTextPreview('  - [ ]')).toBe('');
      expect(sanitizeForTextPreview('1. [ ]')).toBe('1.');
      expect(sanitizeForTextPreview('2) [x]')).toBe('2)');
      expect(sanitizeForTextPreview('[ ]')).toBe('');
      expect(sanitizeForTextPreview('[x]')).toBe('');
    });

    it('should not strip single-char bracket expressions in prose', () => {
      expect(sanitizeForTextPreview('press [a]b to continue')).toBe(
        'press [a]b to continue'
      );
      expect(sanitizeForTextPreview('option [B].')).toBe('option [B].');
    });
...
Second line
Third line`;
      const result = sanitizeForTextPreview(input, 'always');
    ...
    it('should omit first line when it matches filename (ifMatchesTitle)', () => {
      const input = `My File
Content here`;
      const result = sanitizeForTextPreview(input, 'ifMatchesTitle', 'My File');
      expect(result).toBe('Content here');
    });
...
Content here`;
      const result = sanitizeForTextPreview(
        input,
        'ifMatchesTitle',
        undefined,
        'Page Title'
...
Content here`;
    ...
    it('should never omit first line when omitFirstLine is never', () => {
      const input = `My File
Content here`;
      const result = sanitizeForTextPreview(input, 'never', 'My File');
...
After`;
    ...
    it('should handle block IDs in prose', () => {
      ...
      const result1 = sanitizeForTextPreview(input1);
      ...
      const result2 = sanitizeForTextPreview(input2);
  ...
  describe('loadNotePreview', () => {
    ...
    it('should return property value if valid string', async () => {
      const result = await loadNotePreview(
        mockFile,
        mockApp,
    ...
    it('should return property value if valid number', async () => {
      const result = await loadNotePreview(mockFile, mockApp, 42, {
    ...
    it('should fallback to content when property is null and fallback enabled', async () => {
      mockApp.vault.cachedRead = vi.fn().mockResolvedValue('File content');

      const result = await loadNotePreview(mockFile, mockApp, null, {
        fallbackToContent: true,
        omitFirstLine: 'never',
      });
    ...
    it('should fallback to content when property is undefined', async () => {
      ...
      const result = await loadNotePreview(mockFile, mockApp, undefined, {
        fallbackToContent: true,
        omitFirstLine: 'never',
      });
    ...
    it('should fallback to content when property is empty string', async () => {
      ...
      const result = await loadNotePreview(mockFile, mockApp, '', {
        fallbackToContent: true,
        omitFirstLine: 'never',
      });
    ...
    it('should pass omitFirstLine always to sanitizeForTextPreview', async () => {
      mockApp.vault.cachedRead = vi.fn().mockResolvedValue(`First line
Second line`);
    ...
    it('should pass fileName to sanitizeForTextPreview (ifMatchesTitle)', async () => {
      mockApp.vault.cachedRead = vi.fn().mockResolvedValue(`My File
Content`);
    ...
    it('should pass titleValue to sanitizeForTextPreview (ifMatchesTitle)', async () => {
      mockApp.vault.cachedRead = vi.fn().mockResolvedValue(`Title Here
Content`);
...
**Bold** and *italic* text`);
    ...
    it('should handle read errors gracefully', async () => {
      ...
      await expect(
        loadNotePreview(mockFile, mockApp, null, {
          fallbackToContent: true,
          omitFirstLine: 'never',
        })
    ...
    it('should handle number 0 as valid property value', async () => {
      const result = await loadNotePreview(mockFile, mockApp, 0, {
        fallbackToContent: true,
        omitFirstLine: 'never',
      });
    ...
    it('should thread preserveHeadings to sanitizeForTextPreview', async () => {
      mockApp.vault.cachedRead = vi
        .fn()
    ...
    it('should thread preserveNewlines to sanitizeForTextPreview', async () => {
      mockApp.vault.cachedRead = vi
        .fn()
  ...
  describe('sanitizeForTextPreview with preserveNewlines', () => {
  ...
  describe('sanitizeForTextPreview with both options', () => {
