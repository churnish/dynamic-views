...
export function setupScrollGradients(
  ...
  scrollables.forEach((el) => {
    ...
    // Only update when crossing edge thresholds (start/end) to reduce overhead
    let lastState = { atStart: true, atEnd: false };
    wrapper.addEventListener(
      ...
      () => {
        ...
        // Only update if state changed (crossed a threshold)
        if (atStart !== lastState.atStart || atEnd !== lastState.atEnd) {
          lastState = { atStart, atEnd };
          updateGradientFn(element);
        }
      },
      { passive: true },
