...
export class DynamicViewsCardView extends BasesView {
    ...
    private renderMetadata(
        ...
        if (row1HasContent) {
            ...
            if (!has1 && !has2) {
                ...
                if (prop2Set) {
                    const placeholderContent = field2El.createDiv('meta-content');
                    placeholderContent.textContent = '…';
                }
            } else if (!has1 && has2) {
                // Field 2 has content, field 1 empty - add placeholder ONLY if prop1 is set
                if (prop1Set) {
                    const placeholderContent = field1El.createDiv('meta-content');
                    placeholderContent.textContent = '…';
                }
            }
        ...
        if (row2HasContent) {
            ...
            if (!has3 && !has4) {
                ...
                if (prop4Set) {
                    const placeholderContent = field4El.createDiv('meta-content');
                    placeholderContent.textContent = '…';
                }
            } else if (!has3 && has4) {
                // Field 4 has content, field 3 empty - add placeholder ONLY if prop3 is set
                if (prop3Set) {
                    const placeholderContent = field3El.createDiv('meta-content');
                    placeholderContent.textContent = '…';
                }
            }
        ...
        if (metaEl.children.length === 0) {
            ...
            const field2After = cardEl.querySelector('.meta-field-2') as HTMLElement;
            if (field2After) {
    ...
    private updateScrollGradient(element: HTMLElement): void {
        ...
        const content = element.querySelector('.meta-content') as HTMLElement;
