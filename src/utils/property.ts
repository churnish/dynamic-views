...
export function getFirstBasesPropertyValue(
  ...
  for (const prop of properties) {
    try {
      ...
      // Bases may throw when processing malformed property values (e.g., empty strings in arrays)
      console.warn(`Failed to get property "${prop}":`, e);
      continue;
    }
  }
...
export function getFirstBasesDatePropertyValue(
  ...
  for (const prop of properties) {
    try {
      ...
      // Bases may throw when processing malformed property values (e.g., empty strings in arrays)
      console.warn(`Failed to get date property "${prop}":`, e);
      continue;
    }
  }
...
export function getAllBasesImagePropertyValues(
  ...
  for (const prop of properties) {
    try {
      ...
      // Bases may throw when processing malformed property values (e.g., empty strings in arrays)
      console.warn(`Failed to get image property "${prop}":`, e);
      continue;
    }
  }
...
export function getPropertyLabel(propertyName: string): string {
  ...
  // Strip formula. prefix (added by property getters when property not found)
  // Only strip if this appears to be an auto-added prefix, not a literal property name
