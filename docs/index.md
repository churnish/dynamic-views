...
| [image-loading.md](arch/image-loading.md) | Working on image loading, caching, aspect ratios, broken URL tracking, embed extraction, the content-loader dedup pipeline, or YouTube thumbnails (WebP serving, rung selection by target width, the placeholder width check, probe memoisation) — documents the two-tier cache architecture, fallback chain, load handler wiring, and invariants. |
| [image-viewer.md](arch/image-viewer.md) | Working on image viewer gestures, keyboard handlers, arrow navigation (`viewer-navigation.ts`), constrained vs fullscreen modes, desktop wheel/pointer gestures, mobile touch handling, viewer cleanup, or the YouTube full-resolution upgrade — documents the dual-mode gesture system, keyboard handler map, navigable image set snapshot, leaf guard pattern, cleanup lifecycle, and invariants. |
...
| [image-navigation.md](arch/image-navigation.md) | Working on slideshow navigation, gesture detection, animation sequencing, image preloading, failed image recovery, the external blob cache, or the multi-image corner icon's hidden state (`multi-image-icon.ts`) — documents the navigator state machine, gesture boundary algorithm, undo window, indicator exclusivity and its restore triggers, cleanup lifecycle, and invariants. |
...
| [plugin-view-navigation.md](patterns/plugin-view-navigation.md) | Probing, querying, or targeting Dynamic Views elements via CDP, WebKit Inspector, or DOM queries, or driving an interaction with synthetic events — documents the full view hierarchy, correct selectors, platform-specific probing patterns, and which gestures cannot be synthesised. |
