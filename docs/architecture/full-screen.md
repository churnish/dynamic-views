...
5. **Attach listeners**: `scroll` (passive), `touchstart`/`touchend` (passive) on scrollEl, `touchend` (non-passive) on viewHeaderEl, `resize` on window, `drop` on viewHeaderEl (blocks Obsidian `handleDrop` from opening files when tap shield intercepts native drag events). Set up `ResizeObserver` on search row — serves two roles: `compensateSearchRowResize()` adjusts iOS scrollTop when search toggles during idle/show state, `syncShowLayoutForSearch()` re-syncs Android spacer layout (coalesced via `searchSyncRafId` rAF). `compensateSearchRowResize()` must run before `syncShowLayoutForSearch()` in the callback — the latter sets `programmaticScroll=true`, which would suppress the former.
