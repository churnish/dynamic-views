...
4. `updateGroupOffsetsSynthetic(oldContainerHeightsFastPath)` — compute new offsets from cumulative container height deltas (zero DOM reads). Falls back to `updateCachedGroupOffsets()` if synthetic bails. `syncVirtualScroll({ scrollTop, paneHeight })` with pre-read scroll metrics.
...
5. **`cachedGroupOffsets` must be refreshed before every `syncVirtualScroll()`.** Two paths: (a) `updateCachedGroupOffsets()` — full `.clear()` + rebuild via `getBoundingClientRect` per group container, used when group structure may have changed; (b) `updateGroupOffsetsSynthetic()` — computes new offsets from cumulative container height deltas (zero DOM reads), used in `remeasureAndReposition` where group structure is stable. The synthetic method bails (sets `groupOffsetsDirty`) if any group offset is unknown; call sites follow with `if (this.groupOffsetsDirty) this.updateCachedGroupOffsets()` as fallback. Stale offsets cause incorrect mount/unmount decisions. All paths refresh offsets before `syncVirtualScroll()` — the proportional resize branch uses `updateGroupOffsetsSynthetic()` (zero DOM reads) with `updateCachedGroupOffsets()` fallback.
