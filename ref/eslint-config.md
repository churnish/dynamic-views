...
description: Config structure, project overrides, and bot vs local differences.
---
...
## Moment.js
Moment is bundled with Obsidian and loaded via `require("moment")`. It must not be added to `package.json` dependencies. The `require` call needs `eslint-disable` for `@typescript-eslint/no-require-imports`, `@typescript-eslint/no-unsafe-assignment`, and `import/no-extraneous-dependencies`.
